...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self) {
        ...
        _userHeaderImage = [[UIImageView alloc] initWithFrame:CGRectMake(10, 5, 50, TableHeader)];
        ...
        _userIntroLbl = [[UILabel alloc] initWithFrame:CGRectMake( TableHeader + 20, 5 + TableHeader/2 , screenWidth - 120, TableHeader/2)];
        ...
        _lbl_liulanliang=[[UILabel alloc] initWithFrame:CGRectMake(offSet_X+50, _replyBtn.frame.origin.y, 150, 20)];
        _lbl_liulanliang.font=[UIFont systemFontOfSize:14];
        _lbl_liulanliang.textColor=[UIColor grayColor];
        _lbl_liulanliang.text=@"2356人浏览过";
        [self.contentView addSubview:_lbl_liulanliang];
        
        _favourImage = [[UIImageView alloc] initWithFrame:CGRectZero];
        _favourImage.image = [UIImage imageNamed:@"dianzan"];
...
#pragma mark - // /////////添加说说view
    ...
    WFTextView *textView = [[WFTextView alloc] initWithFrame:CGRectMake(offSet_X+50, 15 + TableHeader, screenWidth - 2 * offSet_X-50, 0)];
    ...
    textView.frame = CGRectMake(offSet_X+50, 15 + TableHeader, screenWidth - 2 * offSet_X-50, hhhh);
...
#pragma mark - /////// //图片部分
    ...
    for (int  i = 0; i < [ymData.showImageArray count]; i++) {
        ...
        UIImageView *image = [[UIImageView alloc] initWithFrame:CGRectMake(((screenWidth - 310)/4)*(i%3 + 1) + 80*(i%3)+70, TableHeader + 10 * ((i/3) + 1) + (i/3) *  ShowImage_H + hhhh + kDistance + (ymData.islessLimit?0:30), 60, ShowImage_H)];
        ...
        [image sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@",[ymData.showImageArray objectAtIndex:i]]] placeholderImage:[UIImage imageNamed:@"placeholder"]];
...
#pragma mark - /////点赞部分
    ...
    WFTextView *favourView = [[WFTextView alloc] initWithFrame:CGRectMake(offSet_X +50+ 30, TableHeader + 10 + ShowImage_H + (ShowImage_H + 10)*(scale_Y/3) + origin_Y + hhhh + kDistance + (ymData.islessLimit?0:30) + balanceHeight + kReplyBtnDistance, screenWidth - 2 * offSet_X - 80, 0)];
...
//点赞图片的位置
    _favourImage.frame = CGRectMake(offSet_X + 58, favourView.frame.origin.y, (ymData.favourHeight == 0)?0:20,(ymData.favourHeight == 0)?0:20);
...
#pragma mark - ///// //最下方回复部分
    ...
    for (int i = 0; i < ymData.replyDataSource.count; i ++ ) {
        ...
        WFTextView *_ilcoreText = [[WFTextView alloc] initWithFrame:CGRectMake(offSet_X+50,TableHeader + 10 + ShowImage_H + (ShowImage_H + 10)*(scale_Y/3) + origin_Y + hhhh + kDistance + (ymData.islessLimit?0:30) + balanceHeight + kReplyBtnDistance + ymData.favourHeight + (ymData.favourHeight == 0?0:kReply_FavourDistance), screenWidth - offSet_X * 2-50, 0)];
        ...
        _ilcoreText.frame = CGRectMake(offSet_X+50,TableHeader + 10 + ShowImage_H + (ShowImage_H + 10)*(scale_Y/3) + origin_Y + hhhh + kDistance + (ymData.islessLimit?0:30) + balanceHeight + kReplyBtnDistance + ymData.favourHeight + (ymData.favourHeight == 0?0:kReply_FavourDistance), screenWidth - offSet_X * 2-50, [_ilcoreText getTextHeight]);
    ...
    if (ymData.replyDataSource.count == 0) {//没回复的时候
        ...
        replyImageView.frame = CGRectMake(offSet_X+50, backView_Y - 10 + balanceHeight + 5 + kReplyBtnDistance, 0, 0);
        ...
        replyImageView.frame = CGRectMake(offSet_X+50, backView_Y - 10 + balanceHeight + 5 + kReplyBtnDistance, screenWidth - offSet_X * 2-50, backView_H + 20 - 8);//微调
        
        _replyBtn.frame = CGRectMake(screenWidth - offSet_X - 40 + 6, replyImageView.frame.origin.y - 24, 40, 18);
    }
    _lbl_liulanliang.text=tempDate.messageBody.liulanliang;
    _lbl_liulanliang.frame=CGRectMake(offSet_X+50, _replyBtn.frame.origin.y, 150, 20);
