...
-(void)startUploadImg
{
    if (_lbl_borndate.text&&_txt_Name.text&&_txt_address.text) {
        ...
        [dataprovider UpLoadIDCardImg:imgData andkey:userinfoWithFile[@"key"]];
    }
    else
    {
...
-(void)UploadImgBackCall:(id)dict
{
    ...
    if (!dict[@"datas"][@"error"]) {
        ...
        {
            NSDictionary * prm=@{@"key":userinfoWithFile[@"key"],@"truename":self.txt_Name.text,@"birthday":self.lbl_borndate.text,@"address":self.txt_address.text,@"idcard":[NSString stringWithFormat:@"%@,%@",imgarray[0],imgarray[1]]};
            DataProvider * dataprovider=[[DataProvider alloc] init];
